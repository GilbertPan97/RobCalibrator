...
namespace Calibrator
{
    ...
    namespace LineScanner
    {
        ...
        HandEyeCalib::HandEyeCalib(Calibrator::CalibType type){
            type_ = type;
        }
        ...
        bool HandEyeCalib::SetRobPose(const std::vector<Eigen::Vector<float, 6>> rob_pose){
            ...
            for (const Eigen::Vector<float, 6>& xyzwpr: vec_end2base_){
                tmp_HTM = Calibrator::Utils::XYZWPRVecToHTM(xyzwpr);
        ...
        bool HandEyeCalib::SetRobPose(const std::vector<Eigen::Vector<float, 6>> rob_pose,
            ...
            Eigen::Matrix4f tmp_HTM;
            for (const Eigen::Vector<float, 6>& xyzwpr: vec_end2base_){
                tmp_HTM = Calibrator::Utils::XYZWPRVecToHTM(ref_frame) * 
                          Calibrator::Utils::XYZWPRVecToHTM(xyzwpr);
                mtr_end2base_.push_back(tmp_HTM);
            }
            return true;
        }

        bool HandEyeCalib::SetObjData(std::vector<cv::Point3f> pnts_data, Calibrator::CalibObj obj){
            ...
            if (obj == Calibrator::CalibObj::SPHERE)
                ctr_pnts_ = pnts_data;
            else if(obj == Calibrator::CalibObj::BLOCK)
        ...
        bool HandEyeCalib::run(SolveMethod method){
            ...
            } else if(calib_obj_ == Calibrator::CalibObj::BLOCK){
                algorithm algor(Constructor::LINE_PARALLEL, method);
                algor.Simultaneous_Calib(mtr_end2base_, toEigenPoints(ctr_pnts_), Rx, tx);
            }

            // save calib result to mtr_cam2rob_ and vec_cam2rob_
            mtr_cam2rob_ = Calibrator::Utils::RT2HomogeneousMatrix(Rx, tx);
            vec_cam2rob_ = Calibrator::Utils::HTMToXYZWPRVec(mtr_cam2rob_);
