...
QRegExp QtPathPropertyManager::regExp(const QtProperty *property) const
{
    return getData<QRegExp>(d_ptr->m_values, &QtPathPropertyManagerPrivate::Data::regExp, property, QRegExp());
}
...
void QtPathPropertyManager::setRegExp(QtProperty *property, const QRegExp &regExp)
{
    const QtPathPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
