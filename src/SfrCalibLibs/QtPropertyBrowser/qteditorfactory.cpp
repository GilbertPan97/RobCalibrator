...
class QtPathEditWidget : public QWidget {
    Q_OBJECT
public:
    QtPathEditWidget(QWidget *parent);
public Q_SLOTS:
...
void QtPathEditWidget::setValue(const QString &value)
{
...
{
    QString path = QFileDialog::getOpenFileName(this, tr("Select a file"), QDir::homePath());
    if (!path.isEmpty()) {
        setValue(path);
    }
}
...
{
    QObject* object=q_ptr->sender();
    const EditorToPropertyMap::const_iterator ecend = m_editorToProperty.constEnd();
    for(EditorToPropertyMap::const_iterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
    {
        if(itEditor.key()==object){
            QtProperty *property = itEditor.value();
...
QWidget *QtPathEditorFactory::createEditor(QtPathPropertyManager *manager, QtProperty *property,
        QWidget *parent)
{
