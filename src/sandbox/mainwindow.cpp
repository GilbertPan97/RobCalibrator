...
MainWindow::MainWindow(QWidget* par) : SARibbonMainWindow(par), m_customizeWidget(nullptr)
{
    ...
    // createQVTKWidget();
    m_3Dviewer = new thQVTKOpenGLNativeWidget(this);
    ads::CDockWidget* DockWidget = new ads::CDockWidget(QString("Image Viewer %1"));
    DockWidget->setWidget(m_3Dviewer);
    this->setCentralWidget(DockWidget);

    std::vector<cv::Point3f> pointCloud;
    cv::FileStorage fs("1_3C_line.yml", cv::FileStorage::READ);
    fs["scan_line"] >> pointCloud;
    fs.release();

    m_3Dviewer->displayPoints(pointCloud);
    ...
    cat_3dScanner->setCategoryName(tr("3D Scanner"));
    cat_3dScanner->setObjectName(("3dScanner"));
    createCategoryOther(cat_3dScanner);
    ribbon->addCategoryPage(cat_3dScanner);

    //添加删除标签页
    SARibbonCategory* cat_2dCamera = new SARibbonCategory();

    cat_2dCamera->setCategoryName(("2D Camera"));
    cat_2dCamera->setObjectName(("2dCamera"));
    ribbon->addCategoryPage(cat_2dCamera);
    createCategoryDelete(cat_2dCamera);

    //设置applicationButton
    ribbon->applicationButton()->setText(("Online"));
...
bool MainWindow::createQVTKWidget(){
    // add vtk view window
    // m_3Dviewer = new QVTKOpenGLWidget(this);
    m_3Dviewer = new thQVTKOpenGLNativeWidget(this);
    // QSurfaceFormat::setDefaultFormat(QVTKOpenGLNativeWidget::defaultFormat());
    
    // set renderer (backgroud)
    ...
    // add axes on world original point
    vtkSmartPointer<vtkAxesActor> axes = vtkSmartPointer<vtkAxesActor>::New();
    vtkTextProperty *textPropX = axes->GetXAxisCaptionActor2D()->GetCaptionTextProperty();
    vtkTextProperty *textPropY = axes->GetYAxisCaptionActor2D()->GetCaptionTextProperty();
    vtkTextProperty *textPropZ = axes->GetZAxisCaptionActor2D()->GetCaptionTextProperty();

    textPropX->SetColor(colors->GetColor3d("Red").GetData());
    textPropY->SetColor(colors->GetColor3d("Green").GetData());
    textPropZ->SetColor(colors->GetColor3d("Blue").GetData()); 
    axes->Modified();

    axes->SetTotalLength(1.0, 1.0, 1.0);    // 设置轴的总长度
    axes->AxisLabelsOff();
    axes->SetShaftTypeToCylinder();         // 将轴设置为圆柱形
    axes->SetCylinderRadius(0.02);          // 设置轴的半径
    renderer->AddActor(axes);               // add axes actor to renderer
    renderer->ResetCamera();
    renderer->GetActiveCamera()->Zoom(0.4);

    // vtkRenderWindow：基于 MESA （vtk软件支持）的可视化窗口类，不支持硬件加速；
    // vtkGenericOpenGLRenderWindow：基于OpenGL（显卡硬件支持），有gpu硬件加速，Qt 内部也是基于 OpenGL 的
    ...
    if (renderWindow->SupportsOpenGL())
        std::cout << "INFO: OpenGL is supported" << std::endl;
    else 
        std::cout << "ERROR: OpenGL is not supported" << std::endl;

    // vtkSmartPointer<vtkRenderWindowInteractor> interactor = vtkSmartPointer<vtkRenderWindowInteractor>::New();
    // interactor->SetRenderWindow(renderWindow);
    // interactor->Initialize();

    // 创建vtkOrientationMarkerWidget对象
    // vtkSmartPointer<vtkOrientationMarkerWidget> orientationMarker = vtkSmartPointer<vtkOrientationMarkerWidget>::New();
    // orientationMarker->SetOrientationMarker(vtkAxesActor::New());
    // orientationMarker->SetInteractor(m_3Dviewer->interactor());
    // orientationMarker->SetViewport(0.0, 0.0, 0.2, 0.2); // Set the viewport to the lower left corner
    // orientationMarker->On();
    // orientationMarker->InteractiveOn();
    // interactor->Start();
    
    m_3Dviewer->setRenderWindow(renderWindow);
    return true;
}
