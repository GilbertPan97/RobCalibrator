...
bool thQVTKOpenGLNativeWidget::displayPoints(std::vector<cv::Point3f> pntCloud)
{
    ...
    vtkSmartPointer<vtkActor> actor_pnts = vtkSmartPointer<vtkActor>::New();
    actor_pnts->SetMapper(mapper);
    actor_pnts->GetProperty()->SetColor(1, 0, 0);           // 红色
    actor_pnts->GetProperty()->SetPointSize(10);            // 点的大小为5个像素
    ...
    this->RenderWindow->GetRenderers()->GetFirstRenderer()->AddActor(actor_pnts);
...
void thQVTKOpenGLNativeWidget::setAxesSystem(vtkRenderWindowInteractor* iren)
{
    ...
    if (m_axesWidget == nullptr)
    {
        ...
        m_axesWidget->SetViewport(0.0, 0.0, 0.3, 0.3);      //��С
